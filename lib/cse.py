...
# Import intent detection for enhanced query building
try:
    from intent_detection import detect_query_intent, get_document_keywords
except ImportError:
    # Fallback for environments where intent_detection is not available
...
def discover_documents(province: str, asset: str, doc_class: str, query: str = None) -> List[str]:
    """
    Discover document URLs using Google Custom Search Engine with intent-based enhancement
    
    Args:
        province: Province code (gd, sd, nm)
        asset: Asset type (solar, coal, wind)
        doc_class: Document class (grid)
        query: Optional user query for intent-based enhancement
    ...
    try:
        ...
        # Get search configuration with intent-based enhancement
        search_queries = _build_search_queries(province, asset, doc_class, query)
...
def _build_search_queries(province: str, asset: str, doc_class: str, user_query: str = None) -> List[str]:
    """
    Build search queries for specific province/asset/doc_class combination with intent-based enhancement
    
    Args:
        province: Province code
        asset: Asset type
        doc_class: Document class
        user_query: Optional user query for intent-based enhancement
    ...
    # Intent-based query enhancement if user query is provided
    intent_keywords = ""
    if user_query:
        try:
            intents = detect_query_intent(user_query)
            intent_keywords = get_document_keywords(intents)
            print(f"Detected intents: {intents}, Keywords: {intent_keywords}")
        except Exception as e:
    ...
    for asset_keyword in asset_keywords:
        for class_keyword in class_keywords:
            # Basic query with intent enhancement
            base_query = f"{province_name} {asset_keyword} {class_keyword}"
            if intent_keywords:
                queries.append(f"{base_query} {intent_keywords}")
            queries.append(base_query)
            
            # With management/regulation terms
            queries.append(f"{base_query} 管理办法")
            queries.append(f"{base_query} 规定")
            queries.append(f"{base_query} 实施细则")
            
            # With specific document types
            queries.append(f"{base_query} 技术要求")
            queries.append(f"{base_query} 申请流程")
...
def _get_secret(secret_name: str) -> Optional[str]:
    """
    Get secret from Google Secret Manager with environment fallbacks.
    ...
    """
    # Environment variable fallbacks for local/testing environments
    env_overrides = {
        'google-api-key': [
            'GOOGLE_API_KEY',
            'GOOGLE_CSE_API_KEY',
        ...
        'google-cse-id': [
            'GOOGLE_CSE_ID',
            'CSE_SEARCH_ENGINE_ID',
            'CSE_ID'
        ]
    }

    for env_key in env_overrides.get(secret_name, []):
    ...
    try:
        ...
        print(f"google-cloud-secret-manager unavailable ({import_error}); using environment fallbacks only")
        return None
